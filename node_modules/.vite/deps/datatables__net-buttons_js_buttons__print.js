...
var _styleToAbs = function(el) {
  var url;
  var clone = (0, import_jquery.default)(el).clone()[0];
  var linkHost;
...
jquery_dataTables_default.ext.buttons.print = {
  ...
  action: function(e, dt, button, config) {
    var data = dt.buttons.exportData(
      import_jquery.default.extend({ decodeEntities: false }, config.exportOptions)
      // XSS protection
    );
    ...
    (0, import_jquery.default)("style, link").each(function() {
      head += _styleToAbs(this);
    });
    try {
      win.document.head.innerHTML = head;
    } catch (e2) {
      (0, import_jquery.default)(win.document.head).html(head);
    }
    win.document.body.innerHTML = "<h1>" + exportInfo.title + "</h1><div>" + (exportInfo.messageTop || "") + "</div>" + html + "<div>" + (exportInfo.messageBottom || "") + "</div>";
    (0, import_jquery.default)(win.document.body).addClass("dt-print-view");
    (0, import_jquery.default)("img", win.document.body).each(function(i2, img) {
