...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Position</h2>
        </template>
                ...
                <div class="overflow-x-auto sm:-mx-6 lg:-mx-8">
                    ...
                    <div class="overflow-hidden">
                        ...
                        <thead
                            ...
                            <tbody>
                                <tr v-for="pos, i in position" :key="pos.id">
                                <td class="border border-gray-400 px-4 py-4">{{ (i+1) }}</td>
                                <td class="border border-gray-400 px-4 py-4">{{ pos.code }}</td>
                                <td class="border border-gray-400 px-4 py-4">{{ pos.name }}</td>
                                <td class="border border-gray-400 px-4 py-4">
                                    <Link :href="route('position.edit',pos.id)"
                                    :class="'px-4 py-2 bg-yellow-400 text-white border rounded-md font-semibold text-xs'">
                                    <i class="fa-solid fa-edit"></i>
                                    </Link>
                                </td>
                                <td class="border border-gray-400 px-4 py-4">
                                    <DangerButton @click="deletePosition(pos.id,pos.name)">
        ...
        <div class="grow-0 shrink-0 basis-auto block w-full lg:flex lg:w-8/12 xl:w-4/12">
            <form @submit.prevent="form.post(route('position.store'))"
                class="mt-6 space-y-6 max-w-xl">
                <InputLabel for="code" value="Code Name"></InputLabel>
